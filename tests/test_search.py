...
def run():
    ...
    test("Combined filters", len(r) > 0 and len(r) <= 3 and all(
    ...
    test("Include tool calls flag", has_tool)

    # 9. Chronological order
    r = discord_recall(db, limit=100)
    timestamps = [x["timestamp"] for x in r]
    test("Chronological order", timestamps == sorted(timestamps))

    # 10. Message IDs preserved
    r = discord_recall(db, channel="office-123", limit=1)
    test("Message IDs preserved", r[0].get("message_id") is not None)

    # === FTS SEARCH ===

    # 11. Basic search
    r = discord_recall(db, search="umbrella")
    test("FTS search 'umbrella'", len(r) > 0 and any("umbrella" in x.get("content", "").lower() for x in r))

    # 12. Multi-word search
    r = discord_recall(db, search="weather Tokyo")
    test("FTS search multi-word", len(r) > 0)

    # 13. Search + channel filter
    r = discord_recall(db, search="umbrella", channel="office-123")
    test("FTS + channel filter", len(r) > 0)

    # 14. Search with no results
    r = discord_recall(db, search="xyznonexistent")
    test("FTS no results", len(r) == 0)

    # 15. Search with FTS-unsafe characters
    r = discord_recall(db, search='test"OR"hack')
    test("FTS injection safe (quotes)", True)  # Shouldn't crash

    r = discord_recall(db, search="test*")
    test("FTS injection safe (wildcard)", True)

    r = discord_recall(db, search="NOT everything")
    test("FTS injection safe (NOT keyword)", True)

    # 16. Search with special model name
    r = discord_recall(db, search="Knirps T.200")
    test("FTS with dots/numbers", len(r) > 0)

    # 17. Empty search
    r = discord_recall(db, search="")
    test("Empty search string", len(r) == 0)

    # === STATS ===

    # 18. Stats
    stats = transcript_stats(db)
    test("Stats entry_count", stats["entry_count"] == 11)
    test("Stats entry_types", "discord_inbound" in stats.get("entry_types", {}))
    test("Stats sources", "discord" in stats.get("sources", {}))

    # === EDGE CASES ===

    # 19. Limit 0
    r = discord_recall(db, limit=0)
    test("Limit 0", len(r) == 0)

    # 20. Limit 1
    r = discord_recall(db, limit=1)
    test("Limit 1", len(r) == 1)

    # 21. Very large limit
    r = discord_recall(db, limit=999999)
    test("Very large limit", len(r) <= 11)  # Can't return more than what exists
    ...
    test("Empty DB recall", len(r) == 0)

    stats = transcript_stats(empty_db)
    test("Empty DB stats", stats["entry_count"] == 0 and stats["session_count"] == 0)
