"""Google Activity / Takeout import planning and file writing."""
...
GOOGLE_ACTIVITY_IMPORT_SCHEMA_VERSION = "chimera-memory.google-activity-import.v1"
GOOGLE_ACTIVITY_IMPORT_TAGS = ["import", "google-activity"]
GOOGLE_ACTIVITY_TEXT_CHAR_LIMIT = 12000

_SLUG_RE = re.compile(r"[^a-z0-9]+")
_HTML_TAG_RE = re.compile(r"<[^>]+>")
_BLOCKING_FINDING_TYPES = {"credential"}
_SUPPORTED_SUFFIXES = {".json", ".html", ".htm", ".txt"}
...
def _slugify(value: str, fallback: str = "google-activity") -> str:
...
def _normalize_time(value: object, fallback: str) -> str:
...
def _string_list(value: object) -> list[str]:
    ...
    if isinstance(value, list):
        return [_clean_text(str(item)) for item in value if str(item or "").strip()]
    text = _clean_text(str(value))
    return [text] if text else []


def _details_text(value: object) -> list[str]:
    details = []
    for item in value if isinstance(value, list) else []:
        if isinstance(item, dict):
            for key in ("name", "value", "text"):
                if item.get(key):
                    details.append(_clean_text(str(item[key])))
        elif item:
            details.append(_clean_text(str(item)))
    return [detail for detail in details if detail]


def _activity_document(source_rel: str, item: dict, index: int, created: str) -> dict | None:
    title = _clean_text(str(item.get("title") or item.get("activity") or item.get("name") or "Google activity"))
    header = _clean_text(str(item.get("header") or item.get("product") or ""))
    title_url = _clean_text(str(item.get("titleUrl") or item.get("url") or item.get("link") or ""))
    products = _string_list(item.get("products") or item.get("product"))
    details = _details_text(item.get("details"))
    created_at = _normalize_time(item.get("time") or item.get("timestamp") or item.get("created_at"), created)
    body_lines = [
        f"Title: {title}",
        f"Header: {header}",
    ...
    if details:
        ...
        body_lines.extend(f"- {detail}" for detail in details)
    body = _clean_text("\n".join(line for line in body_lines if line is not None))
    if not body or body == "Title: Google activity":
        return None
    if len(body) > GOOGLE_ACTIVITY_TEXT_CHAR_LIMIT:
        body = body[:GOOGLE_ACTIVITY_TEXT_CHAR_LIMIT].rstrip() + "\n\n[Truncated by ChimeraMemory Google Activity import.]"
    source_id = str(item.get("id") or _hash_text(f"{source_rel}\n{index}\n{body}"))
    return {
        ...
        "created": created_at,
        "body": body,
        "product": header or (products[0] if products else ""),
    }


def _json_documents(source_rel: str, parsed: object, created: str) -> list[dict]:
    items: list[object]
    if isinstance(parsed, dict):
        for key in ("items", "activities", "activity", "data"):
    ...
    for index, item in enumerate(items):
        if not isinstance(item, dict):
            continue
        document = _activity_document(source_rel, item, index, created)
        if document:
...
def _documents_from_raw(source_rel: str, raw: str, created: str) -> list[dict]:
    ...
    if suffix == ".json":
        try:
            return _json_documents(source_rel, json.loads(raw), created)
        except json.JSONDecodeError:
            return []
    text = _html_to_text(raw) if suffix in {".html", ".htm"} else _clean_text(raw)
    if not text:
        return []
    if len(text) > GOOGLE_ACTIVITY_TEXT_CHAR_LIMIT:
        text = text[:GOOGLE_ACTIVITY_TEXT_CHAR_LIMIT].rstrip() + "\n\n[Truncated by ChimeraMemory Google Activity import.]"
    return [
        {
            "source_path": source_rel,
...
def _iter_google_activity_documents(import_path: Path) -> list[dict]:
    ...
    if path.suffix.lower() == ".zip":
        ...
        raise ValueError("Google Activity import path must be a supported file, directory, or zip export")
    return documents


def render_google_activity_import_markdown(document: dict) -> str:
    """Render one governed Google Activity import memory."""
    title = _clean_text(str(document.get("title") or "Google activity"))
    frontmatter = {
        "type": "episodic",
        "importance": 4,
        ...
        "status": "active",
        "about": title,
        "tags": GOOGLE_ACTIVITY_IMPORT_TAGS,
    ...
    lines.extend(
        [
            ...
            f"- product: {document.get('product') or ''}",
            f"- schema: {GOOGLE_ACTIVITY_IMPORT_SCHEMA_VERSION}",
            "",
            "## Source Content",
            "",
...
def build_google_activity_import_plans(
    import_path: Path,
    *,
    persona: str,
    limit: int = 200,
) -> dict:
    """Build governed markdown import plans from Google Activity exports."""
    persona = persona.strip()
    if not persona:
        return {"ok": False, "error": "persona required"}
    try:
        documents = _iter_google_activity_documents(Path(import_path))
    except (OSError, ValueError, zipfile.BadZipFile) as exc:
        return {"ok": False, "error": f"failed to load Google Activity export: {exc}"}

    plans = []
    for document in documents[: max(0, min(int(limit), 5000))]:
        rendered = render_google_activity_import_markdown(document)
        ...
        slug = _slugify(str(document.get("title") or document.get("source_id") or "google-activity"))
        relative_path = f"memory/imports/google-activity/{date_prefix}-{slug}-{source_hash[:10]}.md"
        plans.append(
            {
                "ok": True,
                "schema_version": GOOGLE_ACTIVITY_IMPORT_SCHEMA_VERSION,
                "source": "google-activity",
...
def write_google_activity_import_file(personas_dir: Path, persona: str, plan: dict, *, force: bool = False) -> dict:
    """Write one planned Google Activity import memory under the persona folder."""
    if not plan.get("ok"):
        return plan
    if plan.get("blocking_findings"):
        return {
            "ok": False,
            "error": "Google Activity import content failed safety scan",
...
def memory_import_google_activity_export(
    ...
    *,
    ...
    plans = build_google_activity_import_plans(Path(import_path), persona=persona, limit=limit)
    if not plans.get("ok"):
        return plans
    ...
    if not write:
        record_memory_audit_event(
            conn,
            "memory_import_google_activity_planned",
            persona=persona,
            target_kind="google_activity_import",
    ...
    pyramid_built = 0
    for plan in plans.get("plans", []):
        result = write_google_activity_import_file(personas_dir, persona, plan, force=force)
        ...
        record_memory_audit_event(
            conn,
            "memory_import_google_activity_document",
            persona=persona,
    ...
    record_memory_audit_event(
        conn,
        "memory_import_google_activity_completed",
        persona=persona,
        target_kind="google_activity_import",
        target_id=str(import_path),
        payload={
