...
Storage (``~/.chimera-memory/auth/google_oauth.json``, chmod 0o600):
...
def _credentials_path() -> Path:
    return Path.home() / ".chimera-memory" / "auth" / "google_oauth.json"
...
def run_gemini_oauth_login_pure(*, project_id: str = "") -> Dict[str, Any]:
    """Run the login flow and return a dict matching the credential pool shape."""
    if not _confirm_google_oauth_policy():
        raise GoogleOAuthError("Google OAuth login cancelled.", code="google_oauth_cancelled")
    creds = start_oauth_flow(force_relogin=True, project_id=project_id.strip() or resolve_project_id_from_env())
    return {
...
def _confirm_google_oauth_policy() -> bool:
    """Hermes CLI consent gate for Gemini CLI OAuth usage."""
    print()
